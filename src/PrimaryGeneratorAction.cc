...
def set_source(new_point, rotationX, rotationZ, thetaDeg, gunAxis):
	...
	G4double theta=thetaDeg*3.14159265/180.0;
...
//#	coords for Gun position
	...
	G4double smallestTubeHeight = 32.9438 * mm;
...
{
	std::vector<std::vector<G4double> > sources(30, std::vector<G4double>(4));
...
//#		for conv_point in conv_point_list:
		...
		//### translation and rotation
			G4double x = std::sin( sources[i][1] *deg) * ( - (sources[i][2]-1.5 ) );
			G4double y = std::sqrt( ( sources[i][2]-1.5 )*(sources[i][2]-1.5 ) - x*x );
			G4double z = -sources[i][3];
			...
			G4double source_distance = std::sqrt(x*x+y*y+z*z);
			G4ThreeVector new_point(x,y,z);
			//#print "HAHO",x,y,z, x*x+y*y+z*z
			G4ThreeVector gunAxis(-x/source_distance, -y/source_distance, -z/source_distance );
